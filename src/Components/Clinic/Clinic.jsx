...
function Clinic() {
  ...
  const [showEmergencyTreatmentInput, setShowEmergencyTreatmentInput] = useState(false);
  ...
  const handleViewRecord = async (record) => {
    ...
    try {
      const labResponse = await axios.get(`http://localhost:3001/api/laboratory?clinicId=${record._id}`);
      setSelectedLabTests(labResponse.data);

      const xrayResponse = await axios.get(`http://localhost:3001/api/xrayResults?clinicId=${record._id}`);
                            ...
                            {dropdownIndex === index && (
                              <div className="absolute right-0 w-40 bg-white rounded-md shadow-lg border z-10">
                                <button
                                  ...
                                  onClick={() => navigate(`/patients/${record.patient._id}`)}
                                >
                                  View Patient
                                </button>
                                <button
