...
const LabXrayCensus = ({ isOpen, onClose, xrayRecords, fromMonthYear, toMonthYear}) => {
  ...
  const generatePDF = async () => {
    ...
    // List of xray descriptions to match
    ...
    while (currentMonth <= toDate) {
      ...
      // Initialize counts for each xray description in the current month
      const monthCounts = xrayDescriptions.reduce((counts, description) => {
        counts[description] = 0;
        return counts;
      }, {});
  
      // Filter xrayRecords by the current month and year
      xrayRecords.forEach(record => {
        ...
        if (recordDate.getMonth() === currentMonth.getMonth() && recordDate.getFullYear() === currentMonth.getFullYear()) {
          // Increment the count for the matching xrayDescription
          if (xrayDescriptions.includes(record.xrayDescription)) {
            monthCounts[record.xrayDescription]++;
          }
        }
      });
      ...
      content.innerHTML += `
        ...
        <div style="margin: 0.5in; font-family: 'Times New Roman', Times, serif; font-size: 10pt;">
          ...
          <table style="width: 100%; border-collapse: collapse; border: 1px solid #000; margin-top: 50px;">
            <thead>
              <tr>
                <th style="padding: 8px; text-align: left; border: 1px solid #000;">Month Year</th>
                ${xrayDescriptions.map(description => (
                  `<th style="padding: 8px; text-align: left; border: 1px solid #000;">${description}</th>`
                )).join('')}
              </tr>
            </thead>
            <tbody>
              <tr>
                <td style="padding: 8px; border: 1px solid #000;">${currentMonthYear}</td>
                ${xrayDescriptions.map(description => (
                  `<td style="padding: 8px; border: 1px solid #000;">${monthCounts[description]}</td>`
                )).join('')}
              </tr>
            </tbody>
          </table>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white py-4 px-6 rounded-lg w-4/5 h-4/5 shadow-2xl max-w-5xl overflow-y-auto flex flex-col justify-between relative">
        <h2 className="text-2xl font-semibold">Laboratory Xray Census Preview</h2>
