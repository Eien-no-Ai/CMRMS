...
function Xray() {
  ...
  const handleAddResult = (record) => {
    ...
    setFormData({
      labNo: record.labNo || "",  // Use record's labNo if available
  ...
  const handleSubmitResult = async () => {
    ...
    formDataToSubmit.append('labNo', formData.labNo);
    ...
    try {
      ...
      const updateResponse = await axios.put(`http://localhost:3001/api/xrayResults/${selectedRecord._id}`, formDataToSubmit, {
        headers: { 
      ...
      console.error("Error in submission process:", error.response ? error.response.data : error.message);
    }
  };
  ...
  <div>
    <div className="bg-gray-100 p-4 rounded-lg border border-gray-300 flex justify-center">
      <p className="text-gray-700 flex items-center">
        <span className="mr-2">&#9432;</span> Whole X-ray request list is not shown to save initial load time.
      </p>
    </div>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-white py-4 px-6 rounded-lg w-4/5 h-4/5 shadow-lg max-w-5xl overflow-y-auto flex flex-col relative">
      ...
      {selectedRecord?.xrayResult === "pending" ? (
        ...
        // Display the details of the selected record if it's marked as "done"
        <div className="flex flex-col mb-4">
          <div className="mb-4">
            <p><strong>X-ray No.:</strong> {formData.labNo}</p>
