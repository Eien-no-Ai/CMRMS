...
function SignUp() {
  ...
  const validateForm = () => {
    ...
    if (!password) {
      ...
      setPasswordError('Password must be at least 8 characters long, with lowercase letters and atleast one uppercase.');
      isValid = false;
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      axios.post('http://localhost:3001/register', { firstname, lastname, email, password, confirmPassword })
        .then(result => {
          console.log(result);
          navigate('/login');
        })
